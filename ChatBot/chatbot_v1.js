...
async function identify_user(agent)
{
  ...
  const snap = await client.db("ChatBot").collection("user_table").findOne({acct_num: parseInt(acct_num)});
...
intentMap.set("opening", identify_user);
intentMap.set("opening - custom - custom", report_issue);
intentMap.set("opening - custom", custom_payload);
